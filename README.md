...
- Поле **`accepts_external_waste`**: три состояния — явное «принимает», явное «не принимает», **`NULL` (неизвестно)**, если в тексте/OCR нет надёжного сигнала; миграция Alembic `20260510_0005`. В метаданных кэша реестра считаются отдельно «принимают», «не принимают» и «не определено»; при поиске с точкой на карте в выдачу попадают только записи с явным «принимает».
- Парсер текста реестра: составные слова с дефисом и кавычки в названиях организаций не ломаются при финальной очистке полей «Объект» и «Собственник».
...
- Добавлены расширенные метрики хода импорта в UI (checkpoint-ы, DB snapshot, geocode flush и тайминги стадий extract/parse/merge/geocode).
- Ускорен backend-поиск: SQL-first prefilter, облегченные select без `payload`, subset-загрузка geocode cache, shared HTTP clients и адаптивный лимит OSRM-кандидатов.
- Добавлены backend unit-тесты для оптимизированных search-loader'ов и веток `object_search`.
- Доработана очистка поля `Вид отхода`: агрессивно отрезаются хвосты с оборудованием/юрлицами/адресами, в UI остается только наименование отхода.
...
- учёт признака приёма отходов от других (`accepts_external_waste`: да / нет / неизвестно);
...
- при выборе точки на карте выдаются только объекты с **явным** «принимает от других» (`accepts_external_waste === true`); записи «нет» и «неизвестно» в эту выборку не входят;
...
# Миграции вручную (если нужно; при старте контейнера `api` они уже выполняются)
...
На Windows, если команда `alembic` не находится в PATH, используйте **`python -m alembic`** (из каталога `server`) после активации `.venv` и `pip install -r requirements.txt`.
...
- **`alembic` не распознано в PowerShell**: вызывайте из каталога `server` команду `python -m alembic ...` в активированном `.venv`, либо `docker compose exec api python -m alembic upgrade head`.
